...
class Config:
    ...
    @classmethod
    def from_config_arg(cls, path: Path | None) -> Self:
        if path is None:
    ...
    @classmethod
    def _from_toml_dir(cls, path: Path) -> Self | None:
    ...
    @classmethod
    def from_toml_dir(cls, path: Path) -> Self:
        """Load from a configuration file in the given path."""
    ...
    @classmethod
    def from_toml_discover(cls, path: Path | None = None) -> Self:
    ...
    @classmethod
    def from_toml_path(cls, path: Path) -> Self:
    ...
    @classmethod
    def from_toml_file(cls, f: BinaryIO, pyproject: bool = False) -> Self:
    ...
    @classmethod
    def from_toml_str(cls, toml: str, pyproject: bool = False) -> Self:
        """Load a configuration from a given string."""
        table = tomllib.loads(toml)
        return cls.from_toml_table(table, pyproject)

    @classmethod
    def from_toml_table(cls, table: dict[str, Any], pyproject: bool = False) -> Self:
